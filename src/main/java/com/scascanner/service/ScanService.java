 ...
 * the whole pipeline can be unit tested - and was, during development - without standing up a
 * web server; {@link com.scascanner.api.ScanController} is a thin adapter on top of this.
 *
 * A problem with one dependency - bad input, or the vulnerability source failing for that one
 * package - is recorded as a {@link ScanError} and the rest of the request still completes.
...
public class ScanService {
    ...
    public ScanService(DependencyNormalizer normalizer,
    ...
    public ScanResult scan(ScanRequest request) {
        ...
        for (DependencyInput input : request.dependencies()) {
            ...
            try {
                ...
                errors.add(new ScanError(dependency.canonicalKey(),
                        "could not query vulnerability source: " + e.getMessage()));
                continue;
            }

            List<Finding> rawFindings = correlationEngine.correlate(dependency, candidates);
            List<Finding> dedupedFindings = deduplicator.deduplicate(rawFindings);
            allFindings.addAll(dedupedFindings);
            allPlans.add(remediationEngine.plan(dependency, dedupedFindings));
        }

        ScanResult result = new ScanResult(UUID.randomUUID().toString(), request.project(),
                Instant.now(), allFindings, allPlans, errors);
        store.save(result);
        return result;
    }
    ...
    private String describe(DependencyInput input) {
        if (input == null) return "(null entry)";
        String name = input.name() != null ? input.name() : "(missing name)";
        String version = input.version() != null ? input.version() : "(missing version)";
        return name + "@" + version;
    }
}
