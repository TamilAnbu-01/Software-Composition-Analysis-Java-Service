...
/**
 * Wires the framework-free core (normalize/intel/correlate/dedup/remediate/service) as Spring
 * beans. Deliberately kept separate from the domain classes themselves - none of them import
 * Spring, so they stay usable (and were used, during development) outside a Spring context.
 */
@Configuration
public class BeanConfig {
    ...
    @Bean
    public ScanService scanService(DependencyNormalizer normalizer,
                                    VulnerabilitySource vulnerabilitySource,
                                    CorrelationEngine correlationEngine,
                                    Deduplicator deduplicator,
                                    RemediationEngine remediationEngine,
                                    ScanStore scanStore) {
        return new ScanService(normalizer, vulnerabilitySource, correlationEngine,
                deduplicator, remediationEngine, scanStore);
    }
}
